...
const users: User[] = [
    ...
    { _id: 3, username: 'test', password: 'test', firstName: 'test', lastName: 'User', role: Role.User }
];
...
export class FakeBackendInterceptor implements HttpInterceptor {
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        function handleRoute() {
            switch (true) {
                case url.endsWith('/maintenanceLogTable') && method === 'GET':
                    return getMaintenanceLogs();
                case url.endsWith('/addMaintenanceLog') && method === 'POST':
                    return addMaintenanceLog();
                case url.endsWith('/updateMaintenanceLog') && method === 'PUT':    
        ...
        function addMaintenanceLog(){
            maintenanceLogs.push(new MaintenanceLog({_id:10, question: body}));
